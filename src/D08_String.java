...
public class D08_String {
	...
	public static <languages> void main(String[] args) {
		String str = new String("abcdefg");	//원래 인스턴스 생성은 이렇게 해야하지만
		String str2 = "hello";	//String만 예외로 인스턴스를 이렇게 생성할 수 있다
		...
		// String.join(delimiter, 이어붙이고싶은것...) : 여러 문자열을 원하는 구분자를 추가하여 이어붙인다
		...
		System.out.println("ph 4로 끝나나요?" + ph.endsWith("4"));
		...
		// 어떤 규칙(인코딩 타입)을 사용해 byte[]로 변환할지 경정할 수 있다
		try {
		...
		// indexOf(seq, start) : 시작 위치부터 원하는 문자열의 인덱스(위치)를 반환한다
		...
		// 연습 문제 : languages에서 "Java"라는 문자열의 모든 인덱스를 찾은 뒤
		//			적절한 자료 컬렉션에 저장하여 출력해보세요
