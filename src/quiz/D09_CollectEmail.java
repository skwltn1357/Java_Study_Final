...
public class D09_CollectEmail {
	/*
	 	...
	 	(올바른 이메일이 총 몇개인지 세어보세요)
	 	
	 	중복되는 이메일이 있다면 무엇인지 찾아보세요
	 */

	static String emailChk(String email) {
	...
	public static void main(String[] args) { 
		ArrayList<String> mailList = new ArrayList<String>();
			...
			System.out.println("----------------------------------------------");
			System.out.println("올바른 이메일 개수 : " + mailList.size());
			
			
			int count = 0;
			for (int i = 0; i < mailList.size(); i++) {
				for (int j = 0; j < mailList.size(); j++) {
					if(mailList.get(i) == mailList.get(j)) {
						count++;
					}
				}
				
				if(count >= 1) {
					System.out.println("중복된 이메일 : " + mailList.get(i));
					break;
				}else {
					count = 0;
				}
